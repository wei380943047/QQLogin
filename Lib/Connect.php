...
class Connect
{
    ...
    public function login()
    {
        ...
        $appid = $this->config->getConfig('appid');
        $callback = $this->config->getConfig("callback");
        $scope = $this->config->getConfig("scope");

        //-------生成唯一随机串防CSRF攻击
        $state = md5(uniqid(rand(), TRUE));
        $this->config->write('state', $state);
    ...
    public function callback()
    {
        ...
        $keysArr = array(
            ...
            "client_id" => $this->config->getConfig("appid"),
            "redirect_uri" => urlencode($this->config->getConfig("callback")),
            "client_secret" => $this->config->getConfig("appkey"),
            "code" => $_GET['code']
        );
        ...
        $this->config->write("access_token", $params["access_token"]);
    ...
    public function getOpenid()
    {
        ...
        //-------请求参数列表
        $keysArr = array(
            "access_token" => $this->config->read("access_token")
        );
        ...
        //------记录openid
        $this->config->write("openid", $user->openid);
        return $user->openid;
    ...
    public function __call($name, $arg)
    {
        ...
        if (empty($this->APIMap[$name])) {
            return "api调用名称错误不存在的API: <span style='color:red;'>$name</span>";
        }
        ...
        if ($responseArr['ret'] == 0) {
            ...
            return $response->ret . $response->msg;
        }
